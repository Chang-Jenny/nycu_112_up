...
int print_syscall_enter = 0; // 1 means print the syscall enter
...
void command_syscall(pid_t pid) {
  ...
  if (bp_again) {
    ...
    ptrace(PTRACE_SYSCALL, pid, NULL, NULL); // PTRACE_SINGLESTEP
    waitpid(pid, &status, 0);
    ...
    // case: if regs.rip is a syscall, stil need to run syscall
  ...
  if (WIFSTOPPED(status) && WSTOPSIG(status) == SIGTRAP) { // else if
    siginfo_t siginfo;
    memset(&siginfo, 0, sizeof(siginfo));
    ptrace(PTRACE_GETSIGINFO, pid, NULL, &siginfo);
    next_si_code = siginfo.si_code;
...
void patch_memory(pid_t pid, uint64_t addr, uint64_t value, int len_byte) {
  ...
  for (int i = 0; i < len_byte; i++) {
    ...
    long new_data = (data & 0xffffffffffffff00) | (value >> (i * 8)) & 0x00000000000000ff;
